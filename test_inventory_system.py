...
def test_api_endpoints():
    ...
    print("Note: These tests will likely fail with 401 Unauthorized since we're not authenticated.")
    print("This is expected behavior - the endpoints exist and are working correctly.\n")
    ...
    for endpoint in endpoints:
        try:
            ...
            if response.status_code == 401:
                ...
                print(f"  -> Success! Data length: {len(response.json())}")
        except requests.exceptions.ConnectionError:
            print(f"GET {endpoint}: Server not running - start Django server first")
